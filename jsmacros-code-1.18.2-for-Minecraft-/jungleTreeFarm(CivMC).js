...
const sapling = "minecraft:jungle_sapling";
...
function chop()
{
    ...
    while (p.getPos().z <= zSouth + 0.5)
    {
        if (p.getPos().z >= chopLocation - 4.5 && p.getPos().z <= chopLocation - 3.5)
        {   
            const inv = Player.openInventory();
            
            if (inv.getSlot(selectedHotbarSlot).getDamage() >= 1541)
            {
                Chat.log("Replace");
...
function dumpJungle()
{
    ...
    for (let i = 9; i < 45; i++)
    {
        if (inv.getSlot(i).getItemID() == "minecraft:jungle_log")
        {
            inv.click(i);
            Client.waitTick();
            inv.click(-999);
        ...
        else if (inv.getSlot(i).getItemID() == "minecraft:stripped_jungle_log")
        {
            inv.click(i);
            Client.waitTick();
            inv.click(-999);
        ...
        else if (inv.getSlot(i).getItemID() == "minecraft:jungle_leaves")
        {
            inv.click(i);
            Client.waitTick();
            inv.click(-999);
